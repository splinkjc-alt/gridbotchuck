...
class Block:
    ...
    def write(self, data_or_format, *args):
        data = struct.pack(data_or_format, *args) if len(args) else data_or_format
...
class Allocator:
    ...
    def open(cls, file_or_name, mode="r+"):
        ...
        if "w" in mode:
            ...
            root = b"".join([struct.pack(b">III", 1, 0, 2048 | 5), struct.pack(b">I", 0) * 255, struct.pack(b">I", 0), *free_list])
            f.write(root)
    ...
    def read(self, offset, size_or_format):
        ...
        if fmt is not None:
            ret = struct.unpack_from(fmt, bytes(ret)) if isinstance(ret, bytearray) else struct.unpack(fmt, ret)
    ...
    def _write_root_block_into(self, block):
        ...
        # Free list
        for _w, f in enumerate(self._free):
            block.write(">I", len(f))
            if len(f):
