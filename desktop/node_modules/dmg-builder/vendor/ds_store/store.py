...
class DSStoreEntry:
    ...
    def write(self, block, insert=False):
        ...
        w = block.insert if insert else block.write
...
class DSStore:
                    ...
                    for node in level:
                        ...
                        if prev_pointers is None:
                            ...
                            with store.get_block(ndx) as block:
                                ...
                                for ptr, e in zip(node_ptrs, node, strict=False):
                                    block.write(b">I", ptr)
                                    e.write(block)
    ...
    def _split2(self, blocks, entries, pointers, before, internal):
        ...
        # Write the nodes
        left_block.seek(0)
        next_node = pointers[best_split] if internal else 0
        ...
        right_block.seek(0)
        next_node = pointers[count] if internal else 0
    ...
    def _split3(self, blocks, entries, pointers, before, internal):
        ...
        # Write the nodes
        prev_split = -1
        for block, split in zip(blocks, best_split, strict=False):
            block.seek(0)
            next_node = pointers[split] if internal else 0
    ...
    def _extract(self, blocks, pivots):
        ...
        total = 0
        ppivots = [*pivots, None]
        for b,p in zip(blocks, ppivots, strict=False):
            b.seek(0)
        ...
        with self._get_block(node) as block:
            ...
            if left_node and right_node:
                with self._get_block(left_node) as left, \
                    ...
                    for e,rp in zip(pivots, ptrs[1:], strict=False):
