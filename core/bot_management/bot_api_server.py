...
class BotAPIServer:
    ...
    async def handle_bot_status(self, request):
        ...
        try:
            ...
            if hasattr(self.bot, "balance_tracker") and hasattr(
                self.bot.balance_tracker, "get_adjusted_fiat_balance"
            ):
                balance_tracker = self.bot.balance_tracker

                # Get current price for total value calculation
                current_price = 0.0
                if hasattr(self.bot, "exchange_service") and hasattr(
                    self.bot.exchange_service, "get_current_price"
                ):
                    try:
                        current_price = (
                ...
                try:
                    ...
                    total_value = (
                        float(balance_tracker.get_total_balance_value(current_price))
                        if current_price > 0
                        else 0.0
                    )
            ...
            if hasattr(self.bot, "grid_manager") and hasattr(
                self.bot.grid_manager, "grid_levels"
            ):
                grid_manager = self.bot.grid_manager
                try:
                    status["grid"] = {
                        "central_price": float(grid_manager.central_price)
                        if hasattr(grid_manager, "central_price")
                        else 0.0,
                        "num_grids": len(grid_manager.grid_levels)
                        if hasattr(grid_manager, "grid_levels")
                        else 0,
                        "buy_grids": len(grid_manager.sorted_buy_grids)
                        if hasattr(grid_manager, "sorted_buy_grids")
                        else 0,
                        "sell_grids": len(grid_manager.sorted_sell_grids)
                        if hasattr(grid_manager, "sorted_sell_grids")
                        else 0,
                    }
                except (TypeError, AttributeError):
    ...
    async def handle_bot_metrics(self, request):
        ...
        try:
            ...
            if hasattr(self.bot, "order_manager") and hasattr(
                self.bot.order_manager, "orders"
            ):
                order_manager = self.bot.order_manager
                try:
                    orders = (
                        order_manager.orders
                        if isinstance(order_manager.orders, list)
                        else []
                    )
                    metrics["total_orders"] = len(orders)
                    metrics["open_orders"] = sum(
                        1 for o in orders if hasattr(o, "is_filled") and not o.is_filled
                    )
                    metrics["filled_orders"] = sum(
                        1 for o in orders if hasattr(o, "is_filled") and o.is_filled
                    )
                except (TypeError, AttributeError):
                    pass
            ...
            if hasattr(self.bot, "balance_tracker") and hasattr(
                self.bot.balance_tracker, "total_fees"
            ):
                try:
            ...
            if hasattr(self.bot, "order_manager") and hasattr(
                ...
                try:
                    ...
                    for order in all_orders[:50]:  # Limit to last 50 orders
                        if hasattr(
                            order, "order_id"
                        ):  # Only process if it looks like a real order
                            orders.append(
                                {
                                    "id": order.order_id
    ...
    async def handle_get_config(self, request):
        ...
        try:
            ...
            if hasattr(self.config_manager, "get_base_currency") and hasattr(
                self.config_manager, "get_quote_currency"
            ):
                try:
                    config["trading_pair"] = (
                        f"{self.config_manager.get_base_currency()}/{self.config_manager.get_quote_currency()}"
                    )
                except Exception:
                    pass
        ...
        try:
            if hasattr(self.bot, "multi_pair_manager") and self.bot.multi_pair_manager:
                ...
                if hasattr(self.config_manager, "config") and isinstance(
                    ...
                    try:
                        multi_pair_config = self.config_manager.config.get(
                            "multi_pair", {}
                        )
                        if isinstance(multi_pair_config, dict):
                            ...
                            config = {
                                ...
                                if isinstance(max_pairs_val, int)
                                else 2,
                            }
                    except Exception:
            ...
            self.logger.warning(
                f"JSON serialization error in multi-pair status, returning defaults: {e}"
            )
            return web.json_response(
                {
                    "success": True,
            ...
            if hasattr(self.config_manager, "is_multi_timeframe_analysis_enabled"):
                try:
                    ...
                    return web.json_response(
                        {
                            "enabled": False,
                            "message": "Multi-timeframe analysis configuration not available",
                        }
                    )
    ...
    async def handle_mtf_analyze(self, request):
        ...
        try:
            ...
            if hasattr(self.config_manager, "is_multi_timeframe_analysis_enabled"):
                try:
                    ...
                    return web.json_response(
                        {
                            "success": False,
                            "message": "Multi-timeframe analysis not configured",
                        },
                        status=400,
                    )
            ...
            # Get grid bounds
            if not hasattr(strategy, "grid_manager") or not hasattr(
                strategy.grid_manager, "price_grids"
            ):
                return web.json_response(
                    {
                        "success": False,
                        "message": "Grid manager not properly initialized",
                    },
                    status=400,
                )
            ...
            try:
                ...
                return web.json_response(
                    {"success": False, "message": "Grid price data not available"},
                    status=400,
                )
            ...
            return web.json_response(
                {
                    ...
                    "analysis": {
                        ...
                        if hasattr(result, "primary_trend")
                        else "unknown",
                        "market_condition": result.market_condition.value
                        if hasattr(result, "market_condition")
                        and hasattr(result.market_condition, "value")
                        else "unknown",
                        "grid_signal": result.grid_signal.value
                        if hasattr(result, "grid_signal")
                        and hasattr(result.grid_signal, "value")
                        else "unknown",
